// The browser simulator: real e-ink waveform timing over the same React bundle the Kindle runs,
// on the wasm build of eink-core when pkg/eink_wasm.js is next to the page, otherwise on the
// pure-TS host in mock-eink.ts. `bootSim()` runs the gallery; `eink-ui dev` calls it with the
// app under development as the first page.
...
import { GalleryApp, PAGES, type GalleryPage } from '../apps/gallery/index.js';

const W = 1072;
const H = 1448;

export interface SimOptions {
  /** The app under development, shown first and titled `appName`; the kit's pages follow. */
  app?: (() => ReactElement) | undefined;
...
function mockTextBackend(): { measureText: MeasureText; drawText: DrawText } {
  ...
  const alignX = (align: TextAlign, boxWidth: number, textWidth: number): number => {
...
async function loadHost(hostEl: HTMLElement): Promise<SimulatedEinkHost> {
  try {
  ...
  return createMockEink(mockTextBackend());
}

// ---- boot -----------------------------------------------------------------

export function bootSim(options: SimOptions = {}): void {
  ...
  const pages: readonly GalleryPage[] = options.app
    ? [{ id: 'app--main', group: 'Your app', name: options.appName ?? 'app', render: options.app }, ...PAGES]
    : PAGES;
  ...
  const blitFramebuffer = (fb: Uint8Array): void => {
    const d = image.data;
  ...
  const onPaint = (damage: DamageRect[]): void => {
  ...
  // the gallery app + routing: one app runs the whole time; the page shown is its `page` prop,
  // which the URL hash, the sidebar, the keyboard and the app's own corner tabs drive via show()
  const defaultPage = pages[0]?.id ?? '';

  /** `#/page/<id>`; the older `#/story/<id>` and `#/app/<name>` links still resolve. */
  const parseRoute = (hash: string): string => {
    const m = /^#\/(page|story|app)\/(.+)$/.exec(hash);
    if (!m) return defaultPage;
    const id = m[1] === 'app' ? `apps--${m[2] ?? ''}` : (m[2] ?? '');
    return findPage(id) ? id : defaultPage;
  };
  ...
  const show = (id: string): void => {
    ...
    eink.request_full();
    render(React.createElement(GalleryApp, { page: id, onPage: show, pages }));
  };
  ...
  void loadHost(hostEl).then((host) => {
    eink = host;
    ...
    window.addEventListener('keydown', (ev) => {
      ...
      let key = ev.key;
      // arrows walk the pages; Page Up/Down are the device's page buttons and go to the page shown
    ...
    // for the DevTools console: the host and the pages
